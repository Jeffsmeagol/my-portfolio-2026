...
import type { AnimatedIconHandle } from "@/components/hover-glow-card";
...
export function ExperienceItem({ experience }: ExperienceItemProps) {
	return (
		<div className="space-y-4 py-4">
			<div className="not-prose flex items-center gap-3">
				<div className="flex size-6 shrink-0 items-center justify-center">
					{experience.companyLogo ? (
						...
						<span className="flex size-2 bg-primary/60 ring-4 ring-primary/10" />
					)}
				</div>
			...
			<div className="relative space-y-4 before:absolute before:left-3 before:h-full before:w-px before:bg-primary/45">
...
export function ExperiencePositionItem({
	...
	const icon = isValidElement(position.icon) ? (
		cloneElement(
			position.icon as ReactElement<{ ref?: React.Ref<AnimatedIconHandle> }>,
			{ ref: positionIconRef },
		)
	) : (
		<BriefcaseBusinessIcon ref={positionIconRef} />
	);
	...
	function stopPositionIconAnimation() {
		positionIconRef.current?.stopAnimation?.();
	}
	...
	return (
		<Collapsible
			...
			render={
				<div className="relative bg-background/60 last:before:absolute last:before:h-full last:before:w-1 last:before:bg-background" />
			}
		>
			<CollapsibleTrigger
				onMouseEnter={startPositionIconAnimation}
				onMouseLeave={stopPositionIconAnimation}
				className={cn(
